<template>
  ...
  <div id="lastfm" class="flex flex-wrap md:-mr-8">
      ...
      <div id="lastFM-recent-tracks" class="mb-6 lg:mb-8 lg:h-640">
        ...
        <ul v-if="tracks && tracks.length">
          ...
          <li v-for="(track, index) of tracks.slice(0, 5)" :key="index">
            <img src="../assets/img/lastfm/track-placeholder.svg" :alt='track.name' v-if="track.image[3]['#text'] === 'https://lastfm.freetls.fastly.net/i/u/300x300/2a96cbd8b46e442fc41c2b86b821562f.png'">
            <img :src="track.image[3]['#text']" :alt='track.name' v-else-if="track.image[3] && track.image[3]['#text']">
            ...
            <div class="date track--now-playing" v-if="track['@attr'] && track['@attr'].nowplaying"> Now Playing </div>
            <div class="date timeago" v-else :title="track.date['#text']">
              {{relativeDate(track.date["#text"], "DD MMM YYYY, HH:mm") }}
            </div>
          </li>
    ...
    <div class="w-full md:w-5/12 md:pl-4">
      <h4 class="mb-2 mt-0 opacity-75">
        Top Artists <span class="ml-2 opacity-50 text-sm">Last 30 Days</span>
      </h4>
      <hr class="mt-2 mb-5" />
      <div id="lastFM-top-artists" class="mb-6 lg:mb-8 lg:h-640">
        ...
        <ul v-if="topartists && topartists.length">
          ...
          <li v-for="(artist, index) of topartists.slice(0, 5)" :key="index">
            <img v-bind:src="'https://tse2.mm.bing.net/th?q=%27+' + artist.name + '+%27%20artist+spotify.com&w=300&h=300&c=7&rs=1'" :alt='artist.name'>
            <div class="artist">
...
#lastFM-recent-tracks ul li div.artist {
  @apply col-span-1 row-span-1 col-start-2 row-start-2 self-start truncate;
}

#lastFM-recent-tracks ul li div.date {
  @apply col-span-1 row-span-1 col-start-2 row-start-3 self-start text-sm opacity-75 pt-1;
}

#lastFM-top-artists ul li div.artist {
  @apply col-span-1 row-span-1 col-start-2 row-start-1 self-start pt-1;
}

#lastFM-top-artists ul li div.play-count {
  @apply col-span-1 row-span-1 col-start-2 row-start-2 self-start pt-1 text-sm opacity-75 pt-1;
}
</style>
